...
for instance in range(1,11):
    ...
    [totalDemand] = bestSolutions.loc[bestSolutions['Trial']==0,'Demand']
    [totalRisk] = bestSolutions.loc[bestSolutions['Trial']==0,'Risk']
    ...
    for switchOnOff in product([0,1],repeat=len(theSwitchable)):
        ...
        if secured == True:  
            ...
            df.to_csv('switch_%s_inst%s.csv'%(len(G.nodes()),instance), index=False)
