...
bestSolutions = pd.read_csv('bestSwitchPhase2_%s_inst%s.csv'%(size,instance))
...
timePhase2 = list(bestSolutions['Time'])[len(list(bestSolutions['Time']))-1]
...
for l in theSwitchable:
    ...
    halfSwitch[l] = 0.5
    zeroSwitch[l] = 0
    switch_l = list(bestSolutions['%s'%l])[len(list(bestSolutions['%s'%l])) - 1]
    bestSwitch[l] = switch_l

bestDemand = list(bestSolutions['Demand'])[len(list(bestSolutions['Demand']))-1]
bestRisk = list(bestSolutions['Risk'])[len(list(bestSolutions['Risk']))-1]
bestPhase2 = list(bestSolutions['Phase2'])[len(list(bestSolutions['Phase2']))-1]
[weightDemand] = bestSolutions.loc[bestSolutions['Trial']==0,'weightDemand']
trial = list(bestSolutions['Trial'])[len(list(bestSolutions['Trial']))-1]
